import{s as ot,i as rt}from"./i18n-DehzL1Xz.js";const r={sources:[],layers:[],activeLayer:null,tool:"move",brushSize:16,tolerance:32,canvasW:512,canvasH:512,zoom:1,showGrid:!0,gridSize:128,snap:!1,status:"Abra uma ou mais imagens e extraia pedaços para compor.",extract:null,layerCounter:0};let xe,S,H=null,U=null,me=!1,C=null,Fe=!1;const pe=new Map,$e=[],ct=15;function fe(e){const a=document.createElement("canvas");return a.width=e.width,a.height=e.height,a.getContext("2d").drawImage(e,0,0),a}function N(){$e.push({layers:r.layers.map(e=>({...e,canvas:fe(e.canvas)})),activeLayer:r.activeLayer}),$e.length>ct&&$e.shift()}function Ke(){const e=$e.pop();if(!e){r.status="Nada para desfazer.",F();return}r.layers=e.layers,r.activeLayer=e.activeLayer,r.status="Desfeito.",F()}function re(e){return e.replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"})[a])}function Q(e,a,i){return Math.max(a,Math.min(i,e))}function G(){return r.layers.find(e=>e.id===r.activeLayer)??null}function Ye(e){return r.layerCounter+=1,`${e}_${r.layerCounter}`}function ve(e,a,i){let n=(a-e.x)/e.scale;const s=(i-e.y)/e.scale;return e.flipX&&(n=e.canvas.width-n),{x:n,y:s}}function lt(e,a,i){const n=ve(e,a,i),s=Math.floor(n.x),o=Math.floor(n.y);return s<0||o<0||s>=e.canvas.width||o>=e.canvas.height?0:e.canvas.getContext("2d",{willReadFrequently:!0}).getImageData(s,o,1,1).data[3]}function Re(e,a){for(let i=r.layers.length-1;i>=0;i-=1){const n=r.layers[i];if(n.visible&&lt(n,e,a)>8)return n}return null}function Ze(e){const i=e.getContext("2d",{willReadFrequently:!0}).getImageData(0,0,e.width,e.height).data;let n=e.width,s=e.height,o=-1,c=-1;for(let u=0;u<e.height;u+=1)for(let p=0;p<e.width;p+=1)i[(u*e.width+p)*4+3]>8&&(p<n&&(n=p),u<s&&(s=u),p>o&&(o=p),u>c&&(c=u));if(o<n)return e;const l=document.createElement("canvas");return l.width=o-n+1,l.height=c-s+1,l.getContext("2d").drawImage(e,-n,-s),l}function dt(e,a){xe=a,S=e,F()}function F(){S.innerHTML=`
  ...
  `}function yt(){const e=r.extract,a=r.sources.find(n=>n.id===e.sourceId);if(!a)return"";const i=e.sel;return`
  ...
  `}function D(){if(!H)return;const e=H.getContext("2d"),a=r.zoom;H.width=Math.max(1,Math.ceil(r.canvasW*a)),H.height=Math.max(1,Math.ceil(r.canvasH*a)),e.imageSmoothingEnabled=!1,e.clearRect(0,0,H.width,H.height);for(const n of r.layers)n.visible&&(e.save(),e.translate(n.x*a,n.y*a),e.scale(n.scale*a*(n.flipX?-1:1),n.scale*a),n.flipX&&e.translate(-n.canvas.width,0),e.drawImage(n.canvas,0,0),e.restore());if(r.showGrid&&r.gridSize>1){e.strokeStyle="rgba(154, 170, 189, 0.3)",e.lineWidth=1;for(let n=0;n<=r.canvasW;n+=r.gridSize)e.beginPath(),e.moveTo(n*a+.5,0),e.lineTo(n*a+.5,r.canvasH*a),e.stroke();for(let n=0;n<=r.canvasH;n+=r.gridSize)e.beginPath(),e.moveTo(0,n*a+.5),e.lineTo(r.canvasW*a,n*a+.5),e.stroke()}const i=G();if(i&&i.visible){const n=i.canvas.width*i.scale*a,s=i.canvas.height*i.scale*a;e.strokeStyle="rgba(102, 224, 170, 0.85)",e.lineWidth=1.5,e.setLineDash([5,4]),e.strokeRect(i.x*a,i.y*a,n,s),e.setLineDash([])}if(C){const n=Math.min(C.x0,C.x1)*a,s=Math.min(C.y0,C.y1)*a,o=Math.abs(C.x1-C.x0)*a,c=Math.abs(C.y1-C.y0)*a;e.fillStyle="rgba(255, 107, 107, 0.18)",e.fillRect(n,s,o,c),e.strokeStyle="rgba(255, 107, 107, 0.8)",e.strokeRect(n,s,o,c)}}function bt(){S.querySelectorAll("[data-src-thumb]").forEach(e=>{const a=r.sources.find(l=>l.id===e.dataset.srcThumb);if(!a)return;const i=44;e.width=i,e.height=i;const n=e.getContext("2d");n.imageSmoothingEnabled=!1;const s=Math.min(i/a.image.naturalWidth,i/a.image.naturalHeight),o=a.image.naturalWidth*s,c=a.image.naturalHeight*s;n.drawImage(a.image,(i-o)/2,(i-c)/2,o,c)})}function Je(){S.querySelectorAll("[data-layer-thumb]").forEach(e=>{const a=r.layers.find(l=>l.id===e.dataset.layerThumb);if(!a)return;const i=34;e.width=i,e.height=i;const n=e.getContext("2d");n.imageSmoothingEnabled=!1;const s=Math.min(i/a.canvas.width,i/a.canvas.height),o=a.canvas.width*s,c=a.canvas.height*s;n.drawImage(a.canvas,(i-o)/2,(i-c)/2,o,c)})}function oe(){const e=r.extract;if(!e)return;const a=r.sources.find(o=>o.id===e.sourceId),i=S.querySelector("#ext-canvas");if(!a||!i)return;const n=e.zoom;i.width=Math.ceil(a.image.naturalWidth*n),i.height=Math.ceil(a.image.naturalHeight*n);const s=i.getContext("2d");if(s.imageSmoothingEnabled=!1,s.clearRect(0,0,i.width,i.height),s.drawImage(a.image,0,0,i.width,i.height),e.showGrid&&e.gridCols>0&&e.gridRows>0){const o=a.image.naturalWidth/e.gridCols,c=a.image.naturalHeight/e.gridRows;s.strokeStyle="rgba(240, 184, 90, 0.5)",s.lineWidth=1;for(let l=0;l<=e.gridCols;l+=1)s.beginPath(),s.moveTo(l*o*n+.5,0),s.lineTo(l*o*n+.5,i.height),s.stroke();for(let l=0;l<=e.gridRows;l+=1)s.beginPath(),s.moveTo(0,l*c*n+.5),s.lineTo(i.width,l*c*n+.5),s.stroke()}e.sel&&(s.fillStyle="rgba(102, 224, 170, 0.15)",s.fillRect(e.sel.x*n,e.sel.y*n,e.sel.w*n,e.sel.h*n),s.strokeStyle="rgba(102, 224, 170, 0.9)",s.lineWidth=1.5,s.strokeRect(e.sel.x*n,e.sel.y*n,e.sel.w*n,e.sel.h*n))}function Le(e,a,i,n){const s=ve(e,a,i),o=r.brushSize/2/e.scale,c=e.canvas.getContext("2d");if(n){const l=pe.get(e.id);if(!l)return;c.save(),c.beginPath(),c.arc(s.x,s.y,o,0,Math.PI*2),c.clip(),c.clearRect(s.x-o,s.y-o,o*2,o*2),c.drawImage(l,0,0),c.restore()}else c.save(),c.globalCompositeOperation="destination-out",c.beginPath(),c.arc(s.x,s.y,o,0,Math.PI*2),c.fill(),c.restore()}function wt(e,a,i,n,s){const o=ve(e,Math.min(a,n),Math.min(i,s)),c=ve(e,Math.max(a,n),Math.max(i,s)),l=Math.min(o.x,c.x),u=Math.min(o.y,c.y),p=Math.abs(c.x-o.x),f=Math.abs(c.y-o.y);e.canvas.getContext("2d").clearRect(l,u,p,f)}function xt(e,a,i){const n=ve(e,a,i),s=Math.floor(n.x),o=Math.floor(n.y),c=e.canvas.width,l=e.canvas.height;if(s<0||o<0||s>=c||o>=l)return 0;const u=e.canvas.getContext("2d",{willReadFrequently:!0}),p=u.getImageData(0,0,c,l),f=p.data,y=(o*c+s)*4,b=f[y],d=f[y+1],h=f[y+2],$=f[y+3],w=r.tolerance,X=new Uint8Array(c*l),P=[o*c+s];let q=0;for(;P.length;){const B=P.pop();if(X[B])continue;X[B]=1;const j=B*4,se=f[j]-b,J=f[j+1]-d,Y=f[j+2]-h,W=f[j+3]-$;if(Math.sqrt(se*se+J*J+Y*Y+W*W)>w)continue;f[j+3]=0,q+=1;const le=B%c,de=B/c|0;le>0&&P.push(B-1),le<c-1&&P.push(B+1),de>0&&P.push(B-c),de<l-1&&P.push(B+c)}return u.putImageData(p,0,0),q}function $t(e){const a=e.canvas.width,i=e.canvas.height,n=e.canvas.getContext("2d",{willReadFrequently:!0}),s=n.getImageData(0,0,a,i),o=s.data,c=[0,(a-1)*4,(i-1)*a*4,((i-1)*a+a-1)*4],l=[];for(const f of c){if(o[f+3]<8)continue;const y=[o[f],o[f+1],o[f+2]];l.some(b=>Math.abs(b[0]-y[0])+Math.abs(b[1]-y[1])+Math.abs(b[2]-y[2])<12)||l.push(y)}if(l.length===0)return 0;const u=r.tolerance;let p=0;for(let f=0;f<o.length;f+=4)if(!(o[f+3]<8))for(const y of l){const b=o[f]-y[0],d=o[f+1]-y[1],h=o[f+2]-y[2];if(Math.sqrt(b*b+d*d+h*h)<=u){o[f+3]=0,p+=1;break}}return n.putImageData(s,0,0),p}function Ne(){const e=document.createElement("canvas");e.width=r.canvasW,e.height=r.canvasH;const a=e.getContext("2d");a.imageSmoothingEnabled=!1;for(const i of r.layers)i.visible&&(a.save(),a.translate(i.x,i.y),a.scale(i.scale*(i.flipX?-1:1),i.scale),i.flipX&&a.translate(-i.canvas.width,0),a.drawImage(i.canvas,0,0),a.restore());return e}async function De(e,a){const i=new Image;await new Promise((n,s)=>{i.onload=()=>n(),i.onerror=()=>s(new Error("Falha ao carregar PNG")),i.src=e}),r.sources.push({id:Ye("src"),name:a,image:i}),r.status=`Fonte adicionada: ${a}`,F()}function St(e,a,i){let n=document.createElement("canvas");n.width=Math.max(1,Math.round(a.w)),n.height=Math.max(1,Math.round(a.h));const s=n.getContext("2d");s.imageSmoothingEnabled=!1,s.drawImage(e.image,a.x,a.y,a.w,a.h,0,0,n.width,n.height),i&&(n=Ze(n)),N();const o={id:Ye("layer"),name:`${e.name.replace(/\.png$/i,"")} [${Math.round(a.x)},${Math.round(a.y)}]`,canvas:n,x:Math.max(0,Math.round((r.canvasW-n.width)/2)),y:Math.max(0,Math.round((r.canvasH-n.height)/2)),scale:1,flipX:!1,visible:!0};pe.set(o.id,fe(n)),r.layers.push(o),r.activeLayer=o.id,r.status=`Camada criada (${n.width}×${n.height}).`}function kt(){S.querySelectorAll("[data-tool]").forEach(d=>{d.onclick=()=>{r.tool=d.dataset.tool,F()}});const e=S.querySelector("#comp-brush");e&&(e.oninput=()=>{var h;r.brushSize=Number(e.value);const d=(h=e.parentElement)==null?void 0:h.querySelector(".badge");d&&(d.textContent=`${r.brushSize}px`)});const a=S.querySelector("#comp-tol");a&&(a.oninput=()=>{var h;r.tolerance=Number(a.value);const d=(h=a.parentElement)==null?void 0:h.querySelector(".badge");d&&(d.textContent=String(r.tolerance))});const i=S.querySelector("#comp-snap");i&&(i.onchange=()=>{r.snap=i.checked});const n=S.querySelector("#comp-showgrid");n&&(n.onchange=()=>{r.showGrid=n.checked,D()});const s=S.querySelector("#comp-gridsize");s&&(s.oninput=()=>{r.gridSize=Math.max(1,Number(s.value)),D()});const o=S.querySelector("#comp-undo");o&&(o.onclick=Ke);for(const[d,h]of[["comp-w","canvasW"],["comp-h","canvasH"],["comp-zoom","zoom"]]){const $=S.querySelector(`#${d}`);$&&($.oninput=()=>{const w=Number($.value);h==="zoom"?r.zoom=Q(w||1,.1,8):w>=16&&(r[h]=Math.floor(w)),D()})}const c=S.querySelector("#comp-open-src");c&&(c.onclick=async()=>{const d=await xe.openImage();d&&await De(d.dataUrl,d.name)});const l=S.querySelector("#comp-src-drop");l&&(l.ondragover=d=>{d.preventDefault(),l.classList.add("dragover")},l.ondragleave=()=>l.classList.remove("dragover"),l.ondrop=d=>{var $;d.preventDefault(),l.classList.remove("dragover");const h=Array.from((($=d.dataTransfer)==null?void 0:$.files)??[]);for(const w of h){const X=new FileReader;X.onload=()=>{De(X.result,w.name)},X.readAsDataURL(w)}}),S.querySelectorAll("[data-src-extract]").forEach(d=>{d.onclick=()=>{const h=r.sources.find(w=>w.id===d.dataset.srcExtract);if(!h)return;const $=Q(680/Math.max(h.image.naturalWidth,h.image.naturalHeight),.1,2);r.extract={sourceId:h.id,zoom:Math.round($*20)/20,sel:null,dragStart:null,gridCols:4,gridRows:4,showGrid:!1,trim:!0},F()}}),S.querySelectorAll("[data-src-del]").forEach(d=>{d.onclick=()=>{r.sources=r.sources.filter(h=>h.id!==d.dataset.srcDel),F()}}),S.querySelectorAll("[data-layer]").forEach(d=>{d.onclick=()=>{r.activeLayer=d.dataset.layer,F()}}),S.querySelectorAll("[data-layer-vis]").forEach(d=>{d.onclick=h=>{h.stopPropagation();const $=r.layers.find(w=>w.id===d.dataset.layerVis);$&&($.visible=!$.visible,F())}}),S.querySelectorAll("[data-layer-del]").forEach(d=>{d.onclick=h=>{var $;h.stopPropagation(),N(),r.layers=r.layers.filter(w=>w.id!==d.dataset.layerDel),r.activeLayer===d.dataset.layerDel&&(r.activeLayer=(($=r.layers[r.layers.length-1])==null?void 0:$.id)??null),F()}}),S.querySelectorAll("[data-layer-dup]").forEach(d=>{d.onclick=h=>{h.stopPropagation();const $=r.layers.find(X=>X.id===d.dataset.layerDup);if(!$)return;N();const w={...$,id:Ye("layer"),name:`${$.name} (cópia)`,canvas:fe($.canvas),x:$.x+12,y:$.y+12};pe.set(w.id,fe(pe.get($.id)??$.canvas)),r.layers.push(w),r.activeLayer=w.id,F()}});for(const[d,h]of[["data-layer-up",1],["data-layer-down",-1]])S.querySelectorAll(`[${d}]`).forEach($=>{$.onclick=w=>{w.stopPropagation();const X=$.getAttribute(d),P=r.layers.findIndex(B=>B.id===X),q=P+h;P<0||q<0||q>=r.layers.length||(N(),[r.layers[P],r.layers[q]]=[r.layers[q],r.layers[P]],F())}});S.querySelectorAll("[data-lprop]").forEach(d=>{d.oninput=()=>{const h=G();if(!h)return;const $=d.dataset.lprop,w=Number(d.value);$==="scale"?h.scale=Q(w||1,.05,16):h[$]=w,D()}});const u=S.querySelector("#comp-flip");u&&(u.onclick=()=>{const d=G();d&&(N(),d.flipX=!d.flipX,F())});const p=S.querySelector("#comp-trim-layer");p&&(p.onclick=()=>{const d=G();d&&(N(),d.canvas=Ze(d.canvas),pe.set(d.id,fe(d.canvas)),F())});const f=S.querySelector("#comp-rembg");f&&(f.onclick=()=>{const d=G();if(!d)return;N();const h=$t(d);r.status=h>0?`${h} pixels de fundo removidos.`:"Nenhum fundo sólido detectado nos cantos (ajuste a tolerância).",F()});const y=S.querySelector("#comp-export");y&&(y.onclick=async()=>{const d=Ne(),h=await xe.savePng(d.toDataURL("image/png"),"composicao.png");r.status=h?`PNG salvo: ${h.path}`:"Exportação cancelada.",F()});const b=S.querySelector("#comp-to-slicer");b&&(b.onclick=()=>{const d=Ne();xe.sendToSlicer(d.toDataURL("image/png"),"composicao.png")}),Mt(),At()}function Ge(e){const a=H.getBoundingClientRect();return{x:(e.clientX-a.left)/r.zoom,y:(e.clientY-a.top)/r.zoom}}function Mt(){if(!H)return;H.onmousedown=a=>{const i=Ge(a);if(r.tool==="move"){const n=Re(i.x,i.y)??G();if(!n)return;r.activeLayer=n.id,U={id:n.id,startX:i.x,startY:i.y,origX:n.x,origY:n.y},N(),D()}else if(r.tool==="erase"||r.tool==="restore"){const n=G();if(!n){r.status="Selecione uma camada primeiro.",F();return}Fe||(N(),Fe=!0),me=!0,Le(n,i.x,i.y,r.tool==="restore"),D()}else if(r.tool==="wand"){const n=G()??Re(i.x,i.y);if(!n){r.status="Nenhuma camada sob o cursor.",F();return}r.activeLayer=n.id,N();const s=xt(n,i.x,i.y);r.status=s>0?`Varinha: ${s} pixels removidos.`:"Nada removido (fora da camada?).",F()}else r.tool==="rect-erase"&&(C={x0:i.x,y0:i.y,x1:i.x,y1:i.y});a.preventDefault()},H.onmousemove=a=>{const i=Ge(a);if(U){const n=r.layers.find(s=>s.id===U.id);if(n){let s=U.origX+(i.x-U.startX),o=U.origY+(i.y-U.startY);r.snap&&r.gridSize>1&&(s=Math.round(s/r.gridSize)*r.gridSize,o=Math.round(o/r.gridSize)*r.gridSize),n.x=s,n.y=o,D()}}else if(me){const n=G();n&&(Le(n,i.x,i.y,r.tool==="restore"),D())}else C&&(C.x1=i.x,C.y1=i.y,D())};const e=()=>{if(U&&(U=null,F()),me&&(me=!1,Fe=!1,Je()),C){const a=G();a&&(Math.abs(C.x1-C.x0)>1||Math.abs(C.y1-C.y0)>1)&&(N(),wt(a,C.x0,C.y0,C.x1,C.y1),r.status="Área apagada."),C=null,F()}};H.onmouseup=e,H.onmouseleave=()=>{(me||U||C)&&e()},H.onwheel=a=>{a.preventDefault();const i=a.deltaY<0?1.15:1/1.15;r.zoom=Q(Math.round(r.zoom*i*100)/100,.1,8);const n=S.querySelector("#comp-zoom");n&&(n.value=String(r.zoom)),D()}}function At(){const e=r.extract;if(!e)return;const a=r.sources.find(b=>b.id===e.sourceId),i=S.querySelector("#ext-canvas");if(!a||!i)return;const n=S.querySelector("#ext-close");n&&(n.onclick=()=>{r.extract=null,F()});const s=S.querySelector("#ext-zoom");s&&(s.oninput=()=>{var d;e.zoom=Number(s.value);const b=(d=s.parentElement)==null?void 0:d.querySelector(".badge");b&&(b.textContent=`${e.zoom.toFixed(2)}x`),oe()});const o=S.querySelector("#ext-showgrid");o&&(o.onchange=()=>{e.showGrid=o.checked,oe()});for(const[b,d]of[["ext-cols","gridCols"],["ext-rows","gridRows"]]){const h=S.querySelector(`#${b}`);h&&(h.oninput=()=>{e[d]=Math.max(1,Math.floor(Number(h.value)||1)),oe()})}const c=S.querySelector("#ext-trim");c&&(c.onchange=()=>{e.trim=c.checked});const l=S.querySelector("#ext-add");l&&(l.onclick=()=>{e.sel&&(St(a,e.sel,e.trim),r.extract=null,F())});const u=b=>{const d=i.getBoundingClientRect(),h=i.width/d.width,$=i.height/d.height;return{x:Q((b.clientX-d.left)*h/e.zoom,0,a.image.naturalWidth),y:Q((b.clientY-d.top)*$/e.zoom,0,a.image.naturalHeight)}},p=()=>{const b=S.querySelector("#ext-sel-hint");b&&(b.textContent=e.sel?`Seleção: ${Math.round(e.sel.w)}×${Math.round(e.sel.h)}`:"Arraste para selecionar, ou clique numa célula da grade.");const d=S.querySelector("#ext-add");d&&(d.disabled=!e.sel||e.sel.w<1||e.sel.h<1)},f=b=>{if(!e.dragStart)return;const d=u(b);e.sel={x:Math.min(e.dragStart.x,d.x),y:Math.min(e.dragStart.y,d.y),w:Math.abs(d.x-e.dragStart.x),h:Math.abs(d.y-e.dragStart.y)},oe(),p()},y=b=>{if(window.removeEventListener("mousemove",f),window.removeEventListener("mouseup",y),!e.dragStart)return;const d=u(b),h=Math.abs(d.x-e.dragStart.x)>3||Math.abs(d.y-e.dragStart.y)>3;if(!h&&e.showGrid){const $=a.image.naturalWidth/e.gridCols,w=a.image.naturalHeight/e.gridRows,X=Q(Math.floor(d.x/$),0,e.gridCols-1),P=Q(Math.floor(d.y/w),0,e.gridRows-1);e.sel={x:X*$,y:P*w,w:$,h:w}}else h||(e.sel=null);e.dragStart=null,oe(),p()};i.onmousedown=b=>{e.dragStart=u(b),window.addEventListener("mousemove",f),window.addEventListener("mouseup",y),b.preventDefault()}}let je=!1;function Et(e){je||(je=!0,window.addEventListener("keydown",a=>{var u;if(!e())return;const i=a.target;if(i&&(i.tagName==="INPUT"||i.tagName==="TEXTAREA"))return;if((a.ctrlKey||a.metaKey)&&a.key.toLowerCase()==="z"){a.preventDefault(),Ke();return}const s={v:"move",e:"erase",r:"restore",w:"wand",x:"rect-erase"}[a.key.toLowerCase()];if(s&&!a.ctrlKey&&!a.metaKey){r.tool=s,F();return}if(a.key==="["||a.key==="]"){r.brushSize=Q(r.brushSize+(a.key==="]"?4:-4),2,96),F();return}const o=G();if(!o)return;const c=a.shiftKey?10:1;let l=!0;switch(a.key){case"ArrowLeft":o.x-=c;break;case"ArrowRight":o.x+=c;break;case"ArrowUp":o.y-=c;break;case"ArrowDown":o.y+=c;break;case"Delete":case"Backspace":N(),r.layers=r.layers.filter(p=>p.id!==o.id),r.activeLayer=((u=r.layers[r.layers.length-1])==null?void 0:u.id)??null,F();return;default:l=!1}l&&(a.preventDefault(),D())}))}const Ue=["idle","walk","slash","hurt"],Ce=["fisico","fogo","gelo","raio","caos"],v={image:null,dataUrl:"",imageName:"nenhum arquivo",frameSize:32,faces:"left",tags:{idle:{row:0,from:0,to:3},walk:{row:1,from:0,to:3},slash:{row:2,from:0,to:3},hurt:{row:3,from:0,to:3}},previewTag:"walk",meta:{id:"",name:"",kind:"melee",element:"fisico",hp:30,attack:5,defense:2,attackSpeed:.8,moveSpeed:70,attackRange:40,reactionTime:.5,xpReward:12,goldReward:6,resists:{fisico:0,fogo:0,gelo:0,raio:0,caos:0}},status:"Abra a sprite sheet do monstro para começar."};let Ie,V,Be=0;function zt(e,a){V=e,Ie=a,he()}function I(e,a){return`<label class="mc-field"><span>${e}</span>${a}</label>`}function T(e,a,i=1){return`<input type="number" id="${e}" value="${a}" step="${i}">`}function he(){cancelAnimationFrame(Be);const e=v.meta;V.innerHTML=`
    <div class="mc-shell">
      ...
      <section class="mc-col">
        <div class="card">
          ...
          <div class="mc-grid2">
            ...
            ${I("Elemento",`<select id="mc-element">${Ce.map(s=>`<option ${e.element===s?"selected":""}>${s}</option>`).join("")}</select>`)}
            ${I("HP",T("mc-hp",e.hp))}
          ...
          <div class="mc-grid2">
            ${Ce.map(s=>I(s,T(`mc-res-${s}`,e.resists[s]))).join("")}
          </div>
  ...
  `,V.querySelector("#mc-open").onclick=async()=>{const s=await Ie.openImage();if(!s)return;const o=new Image;o.onload=()=>{v.image=o,v.dataUrl=s.dataUrl,v.imageName=s.name,v.status=`Sheet ${o.width}×${o.height}px carregada. Ajuste frame/tags.`,v.meta.id||(v.meta.id=s.name.replace(/\.png$/i,"").replace(/[^a-z0-9_]/gi,"_").toLowerCase()),he()},o.src=s.dataUrl};const a=(s,o)=>{const c=V.querySelector(`#${s}`);c&&(c.onchange=()=>o(Number(c.value)||0))};a("mc-framesize",s=>v.frameSize=Math.max(8,s));for(const s of Ue)a(`mc-${s}-row`,o=>v.tags[s].row=Math.max(0,o)),a(`mc-${s}-from`,o=>v.tags[s].from=Math.max(0,o)),a(`mc-${s}-to`,o=>v.tags[s].to=Math.max(0,o));const i=V.querySelector("#mc-faces");i.onchange=()=>v.faces=i.value,V.querySelectorAll(".mc-see").forEach(s=>{s.onclick=()=>{v.previewTag=s.dataset.tag,V.querySelectorAll(".mc-see").forEach(o=>o.classList.toggle("active",o===s))}});const n=(s,o)=>{const c=V.querySelector(`#${s}`);c&&(c.onchange=()=>o(c.value))};n("mc-id",s=>v.meta.id=s.replace(/[^a-z0-9_]/gi,"_").toLowerCase()),n("mc-name",s=>v.meta.name=s),n("mc-kind",s=>v.meta.kind=s),n("mc-element",s=>v.meta.element=s),a("mc-hp",s=>v.meta.hp=s),a("mc-attack",s=>v.meta.attack=s),a("mc-defense",s=>v.meta.defense=s),a("mc-attackspeed",s=>v.meta.attackSpeed=s),a("mc-movespeed",s=>v.meta.moveSpeed=s),a("mc-attackrange",s=>v.meta.attackRange=s),a("mc-reactiontime",s=>v.meta.reactionTime=s),a("mc-xpreward",s=>v.meta.xpReward=s),a("mc-goldreward",s=>v.meta.goldReward=s);for(const s of Ce)a(`mc-res-${s}`,o=>v.meta.resists[s]=o);V.querySelector("#mc-export").onclick=Ct,Ft()}function Ft(){const e=V.querySelector("#mc-preview"),a=e==null?void 0:e.getContext("2d");if(!e||!a)return;const i=performance.now(),n=s=>{if(document.body.contains(e)){if(a.clearRect(0,0,e.width,e.height),a.imageSmoothingEnabled=!1,v.image){const o=v.tags[v.previewTag],c=Math.max(1,o.to-o.from+1),l=o.from+Math.floor((s-i)/160)%c,u=Math.min(3,96/v.frameSize),p=v.frameSize*u;a.save(),a.translate(e.width/2,e.height/2),v.faces==="right"&&a.scale(-1,1),a.drawImage(v.image,l*v.frameSize,o.row*v.frameSize,v.frameSize,v.frameSize,-p/2,-p/2,p,p),a.restore(),a.fillStyle="#9aa4b5",a.font="9px sans-serif",a.textAlign="center",a.fillText(`${v.previewTag} · linha ${o.row} · frames ${o.from}-${o.to}`,e.width/2,e.height-4)}else a.fillStyle="#666",a.font="10px sans-serif",a.textAlign="center",a.fillText("sem sheet",e.width/2,e.height/2);Be=requestAnimationFrame(n)}};Be=requestAnimationFrame(n)}async function Ct(){const e=v.meta;if(!v.image||!v.dataUrl){v.status="⚠ Abra a sprite sheet antes de exportar.",he();return}if(!e.id||!e.name){v.status="⚠ Preencha id e nome.",he();return}const a=Math.floor(v.image.width/v.frameSize),i={format:"mini-guild-monster@1",template:{id:e.id,name:e.name,isBoss:!1,kind:e.kind,hp:e.hp,attack:e.attack,defense:e.defense,attackSpeed:e.attackSpeed,moveSpeed:e.moveSpeed,attackRange:e.kind==="ranged"?Math.max(200,e.attackRange):e.attackRange,reactionTime:e.reactionTime,xpReward:e.xpReward,goldReward:e.goldReward,element:e.element,resists:Object.fromEntries(Object.entries(e.resists).filter(([,s])=>s!==0))},sprite:{frameSize:v.frameSize,frames:a,faces:v.faces,tags:{idle:{...v.tags.idle},walk:{...v.tags.walk},slash:{...v.tags.slash},thrust:{...v.tags.slash},shoot:{...v.tags.slash},spellcast:{...v.tags.slash},hurt:{...v.tags.hurt}}},png:v.dataUrl},n=await Ie.saveArtifact(JSON.stringify(i,null,2),`${e.id}.monster.json`);v.status=n!=null&&n.path?`✔ Artefato salvo em ${n.path} — ingira com: node scripts/ingest-monsters.mjs "${n.path}"`:"Salvamento cancelado.",he()}const Qe=(()=>{var e,a;try{return((a=(e=JSON.parse(localStorage.getItem("mgi-save")??"{}"))==null?void 0:e.settings)==null?void 0:a.language)??"pt-BR"}catch{return"pt-BR"}})();ot(Qe);rt(Qe);const k=window.api,t={imageName:"nenhum arquivo",imagePath:"",dataUrl:"",image:null,imageWidth:0,imageHeight:0,cols:4,rows:8,offsetX:0,offsetY:0,gapX:0,gapY:0,frameW:128,frameH:128,outputW:128,outputH:128,padding:8,scalePct:100,shiftX:0,shiftY:0,trimAlpha:!0,currentAnim:"walk",zoom:.35,showGrid:!0,showBounds:!1,showCrosshair:!1,selectedFrame:null,focusedFrame:null,status:"Abra um PNG para começar.",animations:[{id:"idle",name:"Idle",row:0,from:0,to:3,fps:2,pingpong:!0},{id:"walk",name:"Walk",row:1,from:0,to:3,fps:5,pingpong:!1},{id:"rest",name:"Rest",row:3,from:0,to:3,fps:2,pingpong:!0},{id:"happy",name:"Happy",row:4,from:0,to:3,fps:4,pingpong:!1},{id:"sit",name:"Sit",row:7,from:0,to:3,fps:2,pingpong:!0}],frameBounds:{},frameOverrides:{},analysisDone:!1,dragStart:null,sheetScrollLeft:0,sheetScrollTop:0,expandedAnim:null,previewActive:!1,animSpeed:1,animPaused:!1,wizardStep:0,spriteType:"pet-grid",spriteFaces:"left",advancedMode:!1,mode:"slice"},ue=document.getElementById("editor-sprites");let ae,_,We=0;performance.now();let qe=0,Se=performance.now();function O(e,a){return`${e},${a}`}function Z(e){return e.replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"})[a])}function Pt(e){return Z(e)}function ke(e,a,i){return Math.max(a,Math.min(i,e))}function E(e,a,i,n="number",s="1"){return`<div class="field"><label>${e}</label><input id="${a}" type="${n}" step="${s}" value="${i}"></div>`}function be(e,a,i,n){return`<div class="field"><label>${n}</label><input data-anim-field="${e}:${String(a)}" type="number" value="${i}"></div>`}function ne(e,a,i,n){return`<div class="field"><label>${n}</label><input data-anim-custom="${e}:${a}" type="number" value="${i}"></div>`}function Pe(e){t.wizardStep=Math.max(0,Math.min(5,e)),M()}function ye(e){return e===0?!0:t.image!==null}function Xt(e){return["Abrir","Grade","Analisar","Animações","Ajustes","Exportar"][e]??"?"}function Bt(){const e=t.image!==null;return`
    <div class="card wizard-bar">
      <div class="wizard-steps">${[0,1,2,3,4,5].map(i=>{let n="wizard-step";i===t.wizardStep?n+=" active":e&&i>0&&i<t.wizardStep&&(n+=" done"),ye(i)||(n+=" disabled");const s=ye(i);return`<button class="${n}" data-wizard-step="${i}" ${s?"":"disabled"}>${i+1}. ${Xt(i)}</button>`}).join("")}</div>
      <button id="wizard-adv-toggle" class="wizard-adv-toggle">${t.advancedMode?"Modo wizard":"Modo avançado"}</button>
    </div>
  `}function qt(){const e=t.image!==null,i=[{value:"pet-grid",label:"Pet"},{value:"monster-strip",label:"Monstro strip"},{value:"monster-grid",label:"Monstro grid"}].map(s=>{const o=t.spriteType===s.value?"checked":"";return`<label class="check"><input type="radio" name="spriteType" value="${s.value}" ${o}> ${s.label}</label>`}).join(""),n=e?`<div class="group"><div class="group-title">Imagem atual</div>
  ...
  `}function It(){const e=t.spriteType==="monster-strip",a=t.image!==null,i=Math.round(t.frameW),n=Math.round(t.frameH);let s;return e?s=`
      <div class="grid-2">
        ${E("Frame W","frameW",i)}
        ${E("Frame H","frameH",n)}
    ...
    ${ce(1)}
  `}function Wt(){const e=t.image!==null,a=t.cols*t.rows,i=Object.keys(t.frameBounds).length;return`
    <div class="card step-content">
  ...
  `}function Ht(){const e=Object.keys(t.frameOverrides).length;return`
    ...
    ${ce(4)}
  `}function Ot(){const e=t.image!==null,a=t.spriteType!=="pet-grid"?` · Arte olha p/ ${t.spriteFaces==="left"?"esquerda":"direita"}`:"",i=t.spriteType==="monster-strip"?`Tipo: Monster &#183; Strip &#183; ${t.cols} colunas &#183; ${t.animations.length} anima&#231;&#245;es${a}`:`Tipo: ${t.spriteType==="monster-grid"?"Monster":"Pet"} &#183; Grid ${t.cols}&#215;${t.rows} &#183; ${t.animations.length} anima&#231;&#245;es${a}`,n=t.previewActive?'<span class="badge" style="margin-left:6px">&#x25C9; Preview ativo no jogo!</span>':"";return`
    <div class="card step-content">
    ...
    ${ce(5)}
  `}function Rt(){return[qt,It,Wt,Tt,Ht,Ot][t.wizardStep]()}function Lt(){if(!t.image)return'<div class="dim hint">Abra uma imagem primeiro.</div>';const e=[];for(let a=0;a<t.rows;a+=1){const i=[];for(let n=0;n<t.cols;n+=1){const s=O(a,n),o=t.frameOverrides[s],c=o&&(o.shiftX||o.shiftY||o.scalePct),u=!!t.frameBounds[s],p=t.focusedFrame===`${a},${n}`,f=(o==null?void 0:o.shiftX)??0,y=(o==null?void 0:o.shiftY)??0;i.push(`
  ...
  `}function Nt(){switch(t.wizardStep){case 0:return`
      ...
      `}default:return""}}function Dt(){var e,a,i,n;return`
    <div class="card preview-card">
      <div class="preview-box"><canvas id="preview-canvas" width="${t.outputW}" height="${t.outputH}"></canvas></div>
      <div class="preview-meta">
        <div class="anim-buttons">${Ee()}</div>
        <div><b>${((e=K())==null?void 0:e.name)??"-"}</b> <span class="badge">row ${((a=K())==null?void 0:a.row)??"-"} &#183; ${((i=K())==null?void 0:i.from)??0}-${((n=K())==null?void 0:n.to)??0}</span></div>
        <div class="status">${Z(t.status)}</div>
      </div>
    </div>
  `}function Gt(){var e,a,i,n;return`
  ...
  `}function tt(){if(!t.selectedFrame||!t.image)return"";const[e,a]=t.selectedFrame.split(",").map(Number),i=O(e,a),n=t.frameBounds[i],s=t.frameOverrides[i],o=L(a,e),c=n!=null,l=c?n.w:Math.round(o.w),u=c?n.h:Math.round(o.h),p=(s==null?void 0:s.shiftX)??0,f=(s==null?void 0:s.shiftY)??0,y=(s==null?void 0:s.scalePct)??t.scalePct;return`
